...
interface FighterInput {
  ...
  debuffs: readonly BattleModifierRef[];
...
`perks` содержит не более пяти уникальных постоянных перков бойца. `buffs`,
`debuffs` и `injuries` используют тот же интерфейс `BattleModifier`, но передаются
отдельными списками для сохранения семантики входных данных. Одинаковые `buffs` и
`debuffs` допускаются и применяются последовательно. Каждый элемент `injuries`
содержит `bodyPartId` в `params`; две активные травмы одной части тела во входе
недопустимы.
...
определения. Два допустимых одинаковых временных эффекта остаются двумя независимыми
экземплярами; правило уникальности травмы по части тела проверяется отдельно.
...
Для ссылки вида `injury` объектная форма обязательна: `params.bodyPartId` задаёт
часть тела, а `params.injuryInstanceId` позволяет трассировать межбоевой экземпляр.
...
бафа, травмы, снаряжения или состояния и различают независимые экземпляры там, где
их повторы разрешены.

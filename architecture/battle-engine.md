...
`equipment-perk` базового приёма. Каждый редкий или именной комплект обязан создать
от одного до двух дополнительных перков; это правило одинаково для оружия и доспехов.
...
расширений и параметров арены. `fighterClass`, качество, определения комплектов и
...
→ оружейный приём и дополнительные equipment-perk
→ временные эффекты
→ стартовые травмы
→ постоянные перки
...
Постоянный перк, оружейный приём, дополнительный эффект снаряжения, временный
эффект и травма используют один контракт `BattlePerk`.
...
Внутри снаряжения базовый оружейный приём идёт раньше дополнительных перков. Это
...
`instanceId` отличают постоянный перк от оружейного приёма, дополнительного перка
снаряжения, временного
